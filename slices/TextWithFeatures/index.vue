<template>
  <Bounded :collapsible="false" as="section" class="bg-slate-100">
    <div class="grid grid-cols-1 items-start gap-10 md:grid-cols-2 md:gap-10 lg:gap-28">
...
export default {
  ...
  methods: {
    textHTMLSerializer (type, element, content, children) {
      switch (type) {
        case 'heading1':
          return /* html */ `<h2 class="text-4xl md:text-6xl mb-4 last:mb-0">${children.join('')}</h2>`
        case 'heading2':
          return /* html */ `<h2 class="text-2xl md:text-4xl mb-4 last:mb-0">${children.join('')}</h2>`
        default:
    ...
    descriptionHTMLSerializer (type, element, content, children) {
      switch (type) {
        case 'heading3':
          return /* html */ `<h3 class="text-2xl mb-2 last:mb-0">${children.join('')}</h3>`
